...
- **A `## ` inside a code fence is not a section boundary.** Upgrade notes quote markdown at people — v0.9.1's knowledge-base note shows an agent's `## Document Access` block — and a line-anchored `^## ` reads that as a heading, silently ending the section there. The guard stayed green on it because the same parser truncates the current file and the tagged file at the same point: symmetric blindness reads exactly like agreement. `maskFencedBlocks` blanks fenced blocks (preserving length, so indices still address the original) and both `parseUpgradeSections` and `noteHeadings` go through it. When you change either, test with a fenced sample, not only with prose.
