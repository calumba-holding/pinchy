...
Pinchy always listens on port **7777** inside the container. To change the port exposed on the host, set the `PINCHY_PORT` environment variable:

```bash
PINCHY_PORT=3000 docker compose up -d
```
...
The default is `7777`. The `docker-compose.yml` maps `${PINCHY_PORT:-7777}:7777` — only the host-side port is configurable.
