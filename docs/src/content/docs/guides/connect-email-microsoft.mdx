...
<Aside type="note">
  Combining `text` with `unread` or `sinceDays` is best-effort on Microsoft 365:
  Graph can't run full-text search and structural filters together, so Pinchy
  fetches the free-text matches first and then filters that page for unread/date
  client-side, rather than sorting the whole mailbox by date the way a plain
  `unread`/`sinceDays` search does.
</Aside>
