...
Each mounted directory appears as a separate selectable directory when creating an agent.

## How directories appear in the UI

When you create a Knowledge Base agent, Pinchy scans `/data/` for subdirectories and displays them as selectable options. Each mounted directory shows up by name — for example, `/data/hr-policies` appears as "hr-policies".

Only top-level subdirectories of `/data/` are listed. Nested directories within those are accessible to the agent but are not listed separately in the creation form.
