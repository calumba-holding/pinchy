...
| `auth.rate_limited`             | A [rate limit](/reference/agent-provisioning-api/#rate-limiting) on `/api/v1/*` is hit (429): a verified key's own request budget, or an IP's invalid-key-attempt budget. Actor is the key (`actorType: "api_key"`) for the former, `actorType: "system"` for the latter — there's no key id for an attempt that never verified. `detail.reason` is `"api_key"` or `"invalid_api_key"`. Throttled to one row per key/IP per minute, same shape as `auth.scope_denied`.                                                             |
