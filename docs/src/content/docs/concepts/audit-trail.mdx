...
| `auth.csrf_blocked`             | A request was rejected because its `Origin` / `Referer` headers didn't match the host it was addressed to — a state-changing API call, or a WebSocket handshake on `/api/ws`. Detail captures `method`, `pathname`, `origin`, `referer`, `remoteAddress`, `clientAddress` and `clientAddressSource` — see [the two address fields](#the-two-address-fields). Throttled to one row per minute (no credential is needed to trigger it); `suppressedSinceLastEntry` counts the rejections a row stands in for. See [Hardening › CSRF gate](/guides/hardening/#cross-site-request-forgery-csrf-protection).                                                         |
| `auth.host_blocked`             | An API request was rejected by the [domain lock](/guides/domain-lock/) because its `Host` header didn't match the configured domain. Detail captures `method`, `pathname`, `host`, `lockedDomain`, `remoteAddress`, `clientAddress` and `clientAddressSource` — see [the two address fields](#the-two-address-fields). Page requests are not audited — only `/api/` paths, so crawlers probing the raw IP don't bury the rows that matter. Throttled to one row per minute (anyone who can reach the instance can trigger this event without a credential); `suppressedSinceLastEntry` counts the rejections a row stands in for.                               |
...
| `auth.password_changed`         | A signed-in user changed their own password from their profile. Distinct from `auth.password_reset_completed`, which is the invite-token flow — this one required knowing the current password. A wrong current password writes `outcome: failure`.                                                                                                                                                                                                                                                                                                                                                                                                             |
| `auth.scope_denied`             | A verified [Agent Provisioning API](/reference/agent-provisioning-api/) key made a request without a required scope (403). Actor is the key itself (`actorType: "api_key"`), throttled to one row per key per minute — see [Auditing](/reference/agent-provisioning-api/#auditing) for the `suppressedSinceLastEntry` detail.                                                                                                                                                                                                                                                                                                                                   |
| `auth.telegram_pairing_denied`  | A signed-in user's Telegram pairing-code redemption (`POST /api/settings/telegram`) was denied: too many attempts (rate limited, 5 per 10 minutes), an invalid or expired code, or the code's Telegram account is already linked to a different Pinchy user. Detail carries the `reason`. Throttled to one row per user per minute, same as `auth.scope_denied`.                                                                                                                                                                                                                                                                                                |
| `auth.rate_limited`             | A [rate limit](/reference/agent-provisioning-api/#rate-limiting) on `/api/v1/*` is hit (429): a verified key's own request budget, or an IP's invalid-key-attempt budget. Actor is the key (`actorType: "api_key"`) for the former, `actorType: "system"` for the latter — there's no key id for an attempt that never verified. `detail.reason` is `"api_key"` or `"invalid_api_key"`. The key-budget rows are throttled to one per key per minute, same shape as `auth.scope_denied`; the invalid-key rows to one per minute in total, because nobody needs a credential to cause one and a per-address window would still let a botnet write a row per host. |

#### The two address fields

`auth.csrf_blocked` and `auth.host_blocked` record two addresses, and they answer different questions.

`remoteAddress` is the machine that opened the connection. Behind a reverse proxy — which is every production install — that is the proxy, so it is the same value on every row. It is kept because it has always meant exactly that, and because on a direct install it _is_ the visitor.

`clientAddress` is who Pinchy believes actually sent the request: the last hop your proxy added to `X-Forwarded-For`, which is the one hop the sender cannot choose. That is the field to drill into when a row makes you ask who was probing.

`clientAddressSource` says which of the two you are looking at:

- `forwarded` — a proxy forwarded it, and it is the visitor's address.
- `socket` — nothing usable was forwarded, so this is just the connection's peer, repeated from `remoteAddress`. On a proxied install that means your proxy is not setting `X-Forwarded-For`; see [`PINCHY_TRUSTED_PROXIES`](/reference/environment-variables/#pinchy_trusted_proxies).
- `unknown` — no address at all, and `clientAddress` is `null`.

An IPv4 address is recorded exactly as it arrived. An **IPv6 address is recorded as its `/64` network** — `2001:db8:1:2:aaaa:bbbb:cccc:dddd` is written as `2001:0db8:0001:0002:0000:0000:0000:0000`. That is the unit Pinchy rate-limits by, because a single visitor routinely owns every address in their `/64`, and it is what `clientAddress` carries so the two always agree. It identifies the visitor's network, not the machine. `remoteAddress` is untouched, so on a direct install the full address is still on the row.
...
| `chat.agent_error`                    | Umbrella event for every chat error chunk plus the silent-stream timeout. Fires **alongside** the specialised events (`agent.model_unavailable`, `chat.silent_stream`), not instead of — so a single SQL query grouped by `detail->>'errorClass'` aggregates every failure shape including the long tail. Not throttled. Detail captures `agent.{id,name}`, `model`, `errorClass` (one of `failover_incomplete_stream` / `schema_rejection` / `model_unavailable` / `model_retired` / `transient` / `provider_config` / `provider_rejected_generic` / `silent_stream_timeout` / `unknown`), and email-scrubbed `providerError` truncated to 1024 bytes. Detail also carries `retried: true` when the umbrella event accompanies a silently-recovered single-shot retry from the OpenClaw dispatch-race wrapper (`chat-dispatch-retry.ts`) — filter on this to separate self-healing transients from user-visible failures ([#355](https://github.com/heypinchy/pinchy/issues/355)). |
...
| `chat.run_aborted`                    | The user stopped an **in-flight** run with the chat composer's **stop** button. Pinchy calls `chatAbort` on OpenClaw so the agent stops generating and the session lane is released for the next turn, then records this event. Detail records `agent.{id,name}`, `sessionKey`, the `runId`, and `reason: "user_request"`. `outcome` is `failure` when the OpenClaw-side abort could not be delivered (e.g. the gateway was briefly unreachable) — the row is still written so a failed abort is auditable too. Actor is the `user`. Only emitted while a run is actually streaming: a stop click with nothing in flight still reaches OpenClaw's abort safely but writes no row (nothing to attribute it to).                                                                                                                                                                                                                                                                      |
...
| `file.upload.attached`                | A staged file was materialised into a chat message. Logged once per file (see [File uploads](#file-uploads) below). On success the detail records `uploadId`, `messageId`, `filename`, and `agent.{id,name}`. On failure (cross-user, already-attached, or expired attachment attempt) the detail records `uploadId` and a `reason`. The file contents themselves are never written to the audit row.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `chat.session_reset`                  | A user cleared their conversation context with an agent (the `/reset` slash command). The OpenClaw session key is reused but starts empty; the transcript stays on disk. Detail records `agent.{id,name}` and the `chatId` the reset targeted (`null` for the agent's default per-user session). Reset is audited and `/compact` is not, because this one makes a running conversation disappear from the user's view.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `chat.image_model_fallback`           | A turn carried an image but the agent's configured model can't see images, so Pinchy ran it on a vision-capable model instead of dropping the attachment silently. Detail records `agent.{id,name}`, the `sessionKey`, the configured `agentModel`, the `fallbackModel` actually used, and `reason: "text_only_model_received_image"`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `file.delivered`                      | An agent handed the user a file it produced — the delivery half of `pinchy_generate_file` (see [Generating files](/guides/file-uploads/#generating-files)). Detail records `agent.{id,name}`, the `filename`, and its `mimeType`. The file's contents are never written to the row.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |

### Knowledge base

| Event Type                    | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `retrieval.query`             | An agent searched the knowledge base with `knowledge_search`. This is the row that answers "which documents did the agent draw on for that answer?" Detail records `agent.{id,name}`, a `queryHash` (the query text itself is never stored), `resultCount`, and `returnedDocumentIds` as `{id, name}` pairs. `includeArchived: true` is set only when the answer drew on archived material, so the common case stays unmarked.                                                                                         |
| `knowledge.reindex`           | An admin (re)indexed an agent's granted folders. Two rows per run, correlated by `jobId`: one when the job is requested (actor: the admin), one when the worker finishes it (actor: `system` / `kb-index-worker`). The outcome row carries `indexed`, `skipped`, `removed`, `unsearchable`, `failed` and `archived`; the request row carries none of them, because zeros would report an empty corpus for every reindex ever started. `pathCount` stands in for the folder paths, which must never reach an audit row. |
| `knowledge.source_viewed`     | Someone opened a cited document from an agent's answer. Detail records `agent.{id,name}` and `document.{name}` — the filename only, never the workspace path or the contents. A view that does not deliver the file writes `outcome: failure` with a `reason` (`outside_allowed_paths`, `not_found`, `not_a_file`, `file_too_large`).                                                                                                                                                                                  |
| `knowledge.source_downloaded` | Someone took a copy of a cited document (`?download=1`). Same read, same gate, same detail shape as `knowledge.source_viewed` — the event type is the only difference, and it is the one an analyst filters on when the question is "who has this file now". Which of the two is written is chosen by the caller, because the response's disposition cannot tell them apart: a `.docx` is served as an attachment either way.                                                                                          |

### Automations

Email workflows the Inbox Agent runs on its own — see [the Automations tab](/concepts/agent-settings/#automations-tab).

| Event Type               | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `email_workflow.created` | A workflow was created. It is always created disabled, so this row is a proposal, not an activation.                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `email_workflow.updated` | A workflow was enabled or disabled. This is the human-gated moment an agent gains or loses standing authority to act on mail by itself — the row worth alerting on.                                                                                                                                                                                                                                                                                                                                                           |
| `email_workflow.deleted` | A workflow was deleted.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `inbox.claim_reset`      | The mail sweep freed a stuck claim: an email a previous run had marked as being worked on but never finished. Freeing it is the sweep's only destructive act and the only way one email gets processed twice, so every freed claim gets its own row. Detail records `workflow.{id,name}` (`"(deleted)"` when the workflow is already gone), the `connectionId`, the `providerMessageId`, and a `sweepId` shared by every row of the same sweep — filter on it to see one sweep as a whole. Actor is `system` / `inbox-sweep`. |

### Tool confirmations

The human-in-the-loop checkpoint in front of a gated tool — see [Require Confirmation Before a Tool Runs](/guides/tool-confirmation). Every row identifies the action by `argsDigest`, a sha256 of the tool's arguments; the arguments themselves are never written to the audit trail, so a confirmation for an email never puts its recipient in the log. All five carry `request.{id}`, `agent.{id,name}`, `requester.{id,name}`, `toolName` and that digest.

| Event Type           | Description                                                                                                                                                                                                                                                                                                                 |
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `approval.requested` | An agent reached a gated tool and was blocked; a confirmation is now waiting for the person driving it. Written once per request, not on the agent's retries — a row here means a human was actually asked.                                                                                                                 |
| `approval.granted`   | The requester approved their own pending confirmation. Adds `approver.{id,name}` (the same person as the requester under self-confirm), an optional `reason`, and `resumed` — see below. This is the row that answers "who cleared this action, and when".                                                                  |
| `approval.denied`    | The requester refused the action. Same detail as `approval.granted`; the agent is told it was denied and does not run the tool.                                                                                                                                                                                             |
| `approval.consumed`  | A grant was spent by a later attempt at the same call. Grants are single-use and bound to one agent, one person, one session, one tool and one exact set of arguments. This is a **fallback** row and rare in practice: an approved call normally resumes inside the agent runtime and never passes the gate a second time. |
| `approval.expired`   | A confirmation nobody answered in time (10 minutes) was closed by the hourly sweep. One row per expired request, sharing a `sweepId` with the rest of the same sweep. The action stays blocked — expiry always fails closed. Actor is `system` / `approval-sweep`.                                                          |

`approval.granted` and `approval.denied` carry **`resumed`**, and it is the field to read when you want to know whether a decision took effect. The agent holds the tool call open while a confirmation is pending, so a decision has to reach it. `resumed: true` means it did, and the agent acted on it. `resumed: false` means the row was written but the call never heard it — the agent had already stopped waiting, or its runtime was briefly unreachable — so the tool did **not** run; `resumeReason` says which, and the row's `outcome` is `failure`. To confirm that an approved action really executed, pair the grant with the `tool.<name>` row the agent writes when the tool runs.
...
| Event Type             | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `file.upload.staged`   | A file was uploaded to the staging area via `POST /api/agents/<agentId>/uploads`. Resource is `agent:<agentId>`. On success the detail records `uploadId`, `filename`, `mimeType`, `sizeBytes`, `contentHash`, and `agent.{id,name}`. On failure (oversize, bad filename, or unsupported MIME) the detail records `filename`, `claimedMime`, a `reason`, and `agent.{id,name}`. Uploads are capped at 15 MB per file — a larger file is rejected with HTTP 413 and a failure entry. |
| `file.upload.attached` | A staged file was promoted into a chat message. Detail shapes are described in the Chat runtime table above.                                                                                                                                                                                                                                                                                                                                                                        |
| `file.upload.expired`  | The hourly GC sweep removed a staged file that was never attached (24-hour staged TTL). Actor is `system` / `upload-gc`. Every row from a single sweep carries the same `sweepId` correlation UUID so an analyst can pull the whole sweep from one drill-down. On success the detail records `uploadId`, `filename`, `sizeBytes`, `agedSeconds`, and `sweepId`. On failure the detail records `uploadId`, `filename`, `sweepId`, and a `reason`.                                    |
...
| `channel.restarted`      | The watchdog restarted a channel account stuck in a Telegram `getUpdates` 409 conflict, so polling resumes as soon as the conflicting poller goes away instead of waiting out OpenClaw's internal retry backoff. Fires when the conflict crosses into `channel.polling_failed`, then at increasing intervals for a bounded number of attempts while it persists — one row per attempt, success or failure. After those attempts a still-unresolved conflict is left to OpenClaw's own retry, so the row count per episode stays bounded. Detail records `channel`, `account.{id,name}`, `reason`, `lastError`, `restartAttempt`, and an `error` on failure. Actor is `system` / `channel-watchdog`. Disable with `TELEGRAM_CONFLICT_AUTO_RESTART=false`. |
| `channel.auto_disabled`  | A recently-connected bot (younger than `TELEGRAM_CONFLICT_RECENT_WINDOW_MS`, default 24 h) whose `getUpdates` 409 conflict survived the watchdog's restarts was disabled automatically — the newcomer backs off so the incumbent deployment keeps the bot. Reconnect it from the agent's Telegram settings once the token is no longer polled elsewhere. Detail records `channel`, `account.{id,name}`, `reason`, and `lastError`. Actor is `system` / `channel-watchdog`. Disable with `TELEGRAM_CONFLICT_AUTO_DISABLE=false`.                                                                                                                                                                                                                          |
| `channel.media_mirrored` | A photo or file sent to a channel (e.g. Telegram) was copied into the agent's workspace `uploads/` folder — one row per file, success or failure. Detail records `channel`, `agent.{id,name}`, `filename`, `mimeType`, `bytes`, and an `error` on failure. See [Attachments](/concepts/attachments/) for the user-facing flow.                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `integration.credentials_denied`  | An agent asked for a connection's credentials without being granted that connection. Written by the internal endpoint the OpenClaw plugins call, so the actor is the **agent**, not a user. A row here means either a misconfigured agent or something in the runtime reaching beyond its grant — both worth a look                                                                                                              | `agent` (`{ id, name }`), `connection` (`{ id }` — a mailbox connection's name is itself an address), `reason` (`not-granted` or `agent-unknown`)                                                                                                     |
| `integration.credentials_tested`  | Mail credentials were probed **before** a connection exists — the IMAP/SMTP test the email connect step runs before it saves anything. Written whether the probe passed or failed, because repeated failures against one host are worth seeing. The verdict is the row's `outcome`; a failure also carries `error.message`. There is no `id` yet, which is why this row identifies the attempt by host rather than by connection | `imapHost`, `smtpHost`, plus `imapCode` / `smtpCode` for whichever leg failed, and `emailHash` / `emailPreview` when the username looks like an address — never the raw address or the password (see [Email addresses](#email-addresses-gdpr-art-17)) |
...
| `config.gated_config_removed` | An admin removed the configuration that only an enterprise license can hold: every group deleted, and the agents whose visibility depended on one reset. Detail records `groupsRemoved`, `agentsReset`, and `{id, name}` snapshots of both (capped, with `truncated` set when there were more). An access-widening action, so the row is written before any follow-up plumbing runs.                                            |
| `settings.updated`            | The locked domain was changed. Resource is `settings:domain`. Detail records a `changes` diff (`{ domain: { from, to } }`); unlocking the domain writes `to: null`.                                                                                                                                                                                                                                                             |
| `settings.deleted`            | A provider was removed from the settings. Resource is `settings:provider:<provider>`. Detail records the provider `name`, the `provider` id, `wasDefault`, `agentCount` (how many agents were reassigned to the remaining provider's default model), the `migratedAgents` list (capped at 10 inline, with `migratedAgentsTruncated` set when there were more), and `newDefault` when the removed provider had been the default. |

### API keys

| Event Type        | Description                                                                                                                                                                 |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `api_key.created` | An [Agent Provisioning API](/reference/agent-provisioning-api/) key was issued. Detail records the key's `id`, `name`, `scopes`, and `expiresAt` — never the plaintext key. |
| `api_key.deleted` | An Agent Provisioning API key was revoked. The key's own row is hard-deleted immediately, so detail snapshots its `name` — the only place it survives afterward.            |

Both events are logged with `actorType: "user"` — a human admin issues and revokes a key through the browser. The key itself acts as `api_key` for everything it does afterward; see [Agent Provisioning API › Auditing](/reference/agent-provisioning-api/#auditing) for that side of the lifecycle, including what happens (and what isn't logged) when a key expires without ever being revoked.

### Audit trail access

| Event Type              | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `audit.exported`        | An admin downloaded an audit-trail export (CSV or PDF). The detail records the chosen format, applied filters, row count, and whether the export was `truncated` by the 100,000-row cap — but never the exported data itself.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `audit.integrity_check` | The periodic background verification job checked a range of the hash chain (see [Automatic verification](#automatic-verification)). Detail records the scanned range (`scannedFrom`/`scannedTo`), the `invalidCount`/`chainBreakCount`, and capped `invalidIds`/`chainBreakIds` lists. It also records `previousKeyCount` and a capped `previousKeyIds` — rows that verified against a superseded `AUDIT_HMAC_SECRET` (see [Rows signed with an earlier secret](/concepts/audit-trail-verification/#rows-signed-with-an-earlier-secret)); those are authentic, so they do not make the run a failure, and this row is the durable record of when a key change happened. `outcome: "success"` means the scanned window was intact; `"failure"` means tampering was detected. |
...
### Actor identity (GDPR Art. 17 crypto-erasure)

Every row logged for a signed-in user carries `actorType: "user"` and an `actorId`. Because that id identifies a person, and the audit log is HMAC-signed and append-only, a raw `users.id` written into `actorId` would — like a raw email address — survive every right-to-erasure request.

Pinchy avoids this with **pseudonymisation**: each user has a random `auditPseudonym` stored on their (mutable) `users` row. When a row is written for that user, Pinchy substitutes the pseudonym for the raw id in `actorId` — before the row is signed, so the hash chain covers the pseudonym consistently. The audit log itself therefore carries no direct identifier: for a `user` row, a human-readable name is resolvable only by joining the separate `users` table (matching either the pseudonym or, for rows written before this feature shipped, the raw id). The admin UI and CSV/PDF exports do that join.

An `api_key` row works differently, and deliberately doesn't join at all. Its name is snapshotted into `detail.apiKey.name` at the moment the row is written, so it survives the key's own revocation — which hard-deletes the key's row outright, leaving nothing left to join against. See [Agent Provisioning API › Auditing](/reference/agent-provisioning-api/#auditing) for the full key lifecycle.

This is what makes **crypto-erasure** possible. Because the only link from a pseudonym back to a person is the `auditPseudonym` value on the mutable `users` row, removing that row (or clearing the column) makes every audit row written for that user unlinkable — the immutable trail stays intact, but nothing in the database can resolve the pseudonym to a name anymore. The admin UI then shows the raw `actorId` as an opaque token, which is the intended erased state, not a display bug.

**Deactivating a user is not the same as erasing them.** Today's _Deactivate_ action bans the account — it is reversible via reactivation and deliberately keeps the `users` row, including `auditPseudonym`, name, and email, so the trail stays attributable while the person still exists in the system. A dedicated, irreversible erasure action that actually removes the mapping is tracked separately ([#697](https://github.com/heypinchy/pinchy/issues/697)). Until it ships, pseudonymisation stands on its own as defence in depth — the log carries no direct identifier — rather than as completed erasure.

Rows written **before** this feature shipped still carry the raw `users.id`. They are not retroactively pseudonymized — the append-only log cannot be rewritten — so erasing that user does not by itself make those older rows unlinkable. Reaching them requires retention (deleting rows past a retention window), a separate feature from crypto-erasure.
...
- **Actor** — name (snapshotted) and ID, plus type (user, agent, system, api_key — a key acting for the organization; the key itself is the actor, not the admin who created it)
...
An export is capped at 100,000 rows. `audit_log` grows without bound, and an unbounded export would read the entire matching set into memory before writing the file — a risk in a resource-constrained container. When more rows match your filters than the cap, the response carries an `X-Audit-Export-Truncated: true` header, the CSV file gets one trailing note line describing the cap, the PDF prints a truncation notice below its header, and the admin UI shows a toast. Narrow the filters (date range, event type, actor) and export again to see the rest.
