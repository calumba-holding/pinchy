...
| `auth.rate_limited`             | A [rate limit](/reference/agent-provisioning-api/#rate-limiting) on `/api/v1/*` is hit (429): a verified key's own request budget, or an IP's invalid-key-attempt budget. Actor is the key (`actorType: "api_key"`) for the former, `actorType: "system"` for the latter — there's no key id for an attempt that never verified. `detail.reason` is `"api_key"` or `"invalid_api_key"`. The key-budget rows are throttled to one per key per minute, same shape as `auth.scope_denied`; the invalid-key rows to one per minute in total, because the address they'd otherwise be keyed on is supplied by the caller. |
...
| `approval.granted`   | The requester approved their own pending confirmation. Adds `approver.{id,name}` (the same person as the requester under self-confirm) and an optional `reason`. This is the row that answers "who cleared this action, and when".                                                                                                      |
...
| `approval.consumed`  | The agent re-issued the call and the approval was spent. Grants are single-use and bound to one agent, one person, one session, one tool and one exact set of arguments, so `granted` followed by `consumed` is the full story of one action running. A `granted` with no `consumed` means the approved action never actually happened. |
| `approval.expired`   | A confirmation nobody answered in time (15 minutes by default) was closed by the hourly sweep. One row per expired request, sharing a `sweepId` with the rest of the same sweep. The action stays blocked — expiry always fails closed. Actor is `system` / `approval-sweep`.                                                           |
