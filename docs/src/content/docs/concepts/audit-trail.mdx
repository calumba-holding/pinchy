...
| `approval.consumed`  | The grant was spent — the agent runtime reported that it let the call through. Grants are single-use and bound to one agent, one person, one session, one tool and one exact set of arguments, so `granted` followed by `consumed` is the story of one action being cleared and then running. Carries `resolution`.      |
| `approval.expired`   | A confirmation that will never be answered. Written by the agent runtime when it stopped waiting (carries `resolution`), or by the hourly sweep for rows nothing reported on (carries a `sweepId` shared with that sweep, actor `system` / `approval-sweep`). The action stays blocked either way — expiry fails closed. |

Two fields answer "did this confirmation actually do anything", from the two ends of the exchange.

**`resumed`**, on `approval.granted` and `approval.denied`, is Pinchy's end: did the decision reach the agent? The agent holds the tool call open while a confirmation is pending, so a decision has to be delivered to it. `resumed: true` means it was. `resumed: false` means the row was written but the call never heard it — the agent had already stopped waiting, or its runtime was briefly unreachable — so the tool did **not** run; `resumeReason` says which, and the row's `outcome` is `failure`.
