...
| `403`  | The agent is not granted this connection, or the connection is not active |
| `404`  | The connection no longer exists                                           |

A refusal writes an `integration.credentials_denied` audit row naming the agent and the connection — see [Audit Trail](/concepts/audit-trail/). A successful fetch is not audited: plugins re-fetch on every cache miss, so a row per success would be volume without signal.
